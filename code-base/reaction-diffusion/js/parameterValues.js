...
export default {
  ...
  // Ensure canvas size is reset when loading settings
  resetCanvasSize(loadedSettings) {
    if (loadedSettings.canvas) {
      this.canvas.width = Math.min(loadedSettings.canvas.width, 4320);
      this.canvas.height = Math.min(loadedSettings.canvas.height, 2480);
      this.resetTextureSizes();
    }
  }
};