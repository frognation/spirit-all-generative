...
import { exportImage, startVideoRecording, stopVideoRecording, isVideoRecording, saveSettings, loadSettings } from '../export';

let pane;
export let currentSeedType = InitialTextureTypes.CIRCLE;

let seedImageChooser;
let videoButton; // reference to update label state
...
function setupActions() {
  ...
  videoButton = actionsFolder.addButton({
    title: '🎬 Start recording'
  });
  videoButton.on('click', () => {
    if (isVideoRecording()) {
      stopVideoRecording();
      videoButton.title = '🎬 Start recording';
      // Trigger download after stopping recording
      const downloadLink = document.createElement('a');
      downloadLink.href = 'path/to/generated/video.mp4'; // Replace with actual video path
  ...
  actionsFolder.addButton({
    ...
    .on('click', () => {
      loadSettings(() => {
        rebuildRightPane();
        // Also rebuild left pane if it exists
        const { rebuildLeftPane } = require('./left-pane');
        if (rebuildLeftPane) rebuildLeftPane();
      });
    });

  // Create ZIP file button
  actionsFolder.addButton({
    title: '📦 Create ZIP of screenshots'
  })
    .on('click', () => {
      const zip = new JSZip();
      const screenshots = globals.screenshots || [];
